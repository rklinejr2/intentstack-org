---
title: "Clause 12 — Open Questions"
weight: 120
---
...
**12.13** How is a proposed Constitutional floor (§2.4) tested for conformance? §2.4 specifies criteria for a well-formed floor but does not specify how a proposed floor is evaluated against them. Self-attestation by the deployer is structurally weak; independent assessment is structurally stronger but unspecified.

**12.14** How are conflicts within a composed Constitutional floor resolved? When a deployer composes a floor from multiple exemplars (§6.2.1), the exemplars may conflict at points of application. This specification does not yet say how conflicts within a composed floor are resolved or which exemplar governs.

**12.15** What makes revision of a Constitutional floor legitimate versus an evasion of the constraint? §2.4.4 requires the floor to bind the deployer, but a floor the deployer authored can in principle be revised by the deployer. The conditions under which a revision counts as legitimate (versus as the deployer escaping the floor) are unspecified.
